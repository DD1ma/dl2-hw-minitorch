...
def tensor_map(
    ...
    * When `out` and `in` are stride-aligned, avoid indexing
    ...
    def _map(
        ...
        if len(in_shape) == len(out_shape) and \
           len(in_strides) == len(out_strides) and \
           np.all(in_shape == out_shape) and np.all(in_strides == out_strides):
...
def tensor_reduce(
    ...
    * Inner-loop should not call any functions or write non-local variables
    ...
    def _reduce(
        ...
        size_reduce_dim = a_shape[reduce_dim]
        for i in prange(len(out)):
            out_index = np.zeros(len(out_shape))
            a_index = np.zeros(len(a_shape))
            to_index(i, out_shape, out_index)
            broadcast_index(out_index, out_shape, a_shape, a_index)
            pos = int(index_to_position(a_index, a_strides))
            result = out[i]
            for j in range(size_reduce_dim):
                result = fn(result, a_storage[pos + j * a_strides[reduce_dim]])
            out[i] = result
