...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        return minitorch.Tensor(v = a._tensor.permute(*order.to_list(int)), backend = a.backend)

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order, ) = ctx.saved_values
        return minitorch.Tensor(v = grad_output._tensor.permute_back(*order.to_list(int)), backend = grad_output.backend), 0.0
